...
import com.joa.prexixion.signer.controller.DashboardApiController;
...
public class StatComportamientoRepository {
    ...
    private static final Logger logger = LoggerFactory.getLogger(StatComportamientoRepository.class);
    ...
    private String calcTrend(BigDecimal actual, BigDecimal anterior) {
        ...
        try {
            ...
            rsp = porcentaje.setScale(2, RoundingMode.HALF_UP).toString();
        } catch (Exception e) {
            logger.error(e.toString());
        }
    ...
    public List<StatComportamiento> getHistoricoEnSoles(String anio, String username) {
        ...
        try {
            ...
            for (String string : periodosList) {
                StatComportamiento stat = new StatComportamiento();
                stat.setPeriodo(
                        DateUtils.getAbrMonthNameCamelCase(string.substring(5, 7)) + "-" + string.substring(2, 4));
